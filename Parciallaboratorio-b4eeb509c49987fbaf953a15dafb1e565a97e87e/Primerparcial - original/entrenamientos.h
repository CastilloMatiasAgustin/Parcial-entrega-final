...
/// Prototipos
int buscarIDentificacion(int IDusuario);
...
int colocacionautomatica();
void listadoentrenamientos(entrenamientos reg);
...
entrenamientos nuevoentrenamiento ()
{
    ...
    reg.identificacion=colocacionautomatica();
...
int colocacionautomatica()
{
    ...
    almacena = ftell(p);
    fclose(p);
    cantidad = almacena / sizeof(entrenamientos);
    return cantidad+1;

}

int buscarIDentificacion (int identificacion)
{
    FILE *p;
    entrenamientos reg;
    int pos=0;
    ...
    while (fread (&reg, sizeof (entrenamientos), 1, p)==1)
    {
        if (reg.identificacion==identificacion)
        {
            fclose (p);
            return pos;
        }
...
void listadoentrenamientos(entrenamientos reg)
{
    FILE *p;
    system ("cls");
    cout<< "LISTADO ENTRENAMIENTOS "<<endl<<endl;
    p=fopen("entrenamientos.dat","rb");
    if (p==NULL)
    {
        cout<< "No se pudo ejecutar el listado "<<endl;
    }
    while (fread(&reg,sizeof (entrenamientos),1,p)==1)
    {
        mostrarentrenamiento(reg);
...
void listarentrenamientoporID()
{
    ...
    FILE *p;
    system ("cls");
    cout<<"Ingrese el id del entrenamiento al cual quiere buscar: "<<endl;
    cin>>IDusuario;
    p=fopen("entrenamientos.dat","rb");
    if (p==NULL)
    {
    ...
    pos=buscarIDentificacion (IDusuario);
    if (pos!=-1){
    fseek(p,sizeof reg*pos,0);
    fread(&reg,sizeof reg,1,p);
    mostrarentrenamiento(reg);
   ...
   cout<< "no existe el ingreso dado "<<endl;
   cin.get();
    system ("pause");
   }
}
